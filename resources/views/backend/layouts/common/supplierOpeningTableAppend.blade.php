    ...
    <table class="table table-bordered table-hover" id="show_item">
        ...
        <tbody>
            ...
            <tr class="new_item<?php echo $key + 99; ?>">
                @if(in_array('supplier_id',$activeColumn))
                <td>
                    <select name="supplier_id[]" class="form-control supplier_id select2 " id=""
                        placeholder="Type Customer">
                        <option value="" selected="" disabled="" data-select2-id="55">(:-Select Customer-:)</option>
                        @foreach($customers as $key => $customer)
                        <option @if($value->supplier_id == $customer->id) selected @endif
            ...
            $selects.find('option').each(function(idx, option) { 
                ...
                function calculation() {
                    ...
                    $('.opening_balance').each(function(i, e) {
                        ...
                        var supplier_id = parseFloat($(this).val() - 0);
                        total_opening_balance += supplier_id;
                    });
